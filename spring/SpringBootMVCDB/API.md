...
도서(Book) CRUD API. `/books` 만 JSP 뷰를 반환하고, 나머지는 `@ResponseBody` 로 JSON 응답한다.
...
### POST /books/update
도서를 수정한다. `bookId` 기준으로 `bookName`, `publisher`, `price` 를 갱신한다.
